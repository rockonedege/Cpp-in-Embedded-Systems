...
template <typename Reg, uint32_t Pos> struct reg_bits_enable_disable
{
    using reg = Reg;
    using T = reg::RegType;
...
template <typename TimerTraits> struct timer
{
    ...
    template <std::uint32_t Period> static void start()
    {
