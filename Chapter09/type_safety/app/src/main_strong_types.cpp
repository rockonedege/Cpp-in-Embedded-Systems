...
tBleStatus aci_gap_create_connection_wrapper(
    conn_l LE_Scan_Interval, conn_l LE_Scan_Window, uint8_t Peer_Address_Type,
    uint8_t Peer_Address[6], conn_p Conn_Interval_Min, conn_p Conn_Interval_Max,
    time Conn_Latency, time Supervision_Timeout, conn_l Minimum_CE_Length,
...
int main()
{
    ...
    const tBleStatus stat = aci_gap_create_connection_wrapper(
        conn_l(80_ms), conn_l(120_ms), c_PUBLIC_ADDR, nullptr, conn_p(50_ms),
        conn_p(60_ms), 0_ms, 4000_ms, conn_l(10_ms), conn_l(15_ms));
